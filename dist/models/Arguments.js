/** @format */
export async function messageArguments(message, command, prefix) {
			...
			if (argument.type === 'list') {
				...
				if (!argument.content) args[argument.name] = items;
					...
					const contents = argument.content.map((content) => content.toLowerCase()),
						itemList = [];
					items.map((item) => (contents.includes(item.toLowerCase()) ? itemList.push(argument.content?.at(contents.indexOf(item.toLowerCase()))) : null));
...
export async function interactionArguments(interaction, command) {
