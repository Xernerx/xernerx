...
import {
  XernerxContextCommandHandlerOptions,
  XernerxMessageCommandHandlerOptions,
  XernerxSlashCommandHandlerOptions,
...
export class CommandHandler extends Handler {
  ...
  protected async loadSlashCommands(
    options: XernerxSlashCommandHandlerOptions,
  ) {
    if (!this.client.modules.options.slash)
      this.client.modules.options.slash = {};
    this.client.modules.options.slash.global =
      options.global ?? this.client.settings.global;

    for (const file of await this.readdir(options.directory)) {
      const command = await this.load(
        path.resolve(`${options.directory}/${file}`),
      );

      if (!command) continue;

      if (
        command.filetype == "XernerxMessageCommand" ||
        command.filetype == "XernerxSlashCommand" ||
        command.filetype == "XernerxContextCommand"
      ) {
        if (!this.client.modules.options.slash.global)
          this.client.stats.commands.slash.local++;
   ...
   */
  protected async loadMessageCommands(
    options: XernerxMessageCommandHandlerOptions,
  ) {}
  protected async loadContextCommands(
    options: XernerxContextCommandHandlerOptions,
  ) {}
}
