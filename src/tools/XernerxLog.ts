...
export default new (class XernerxLog {
  ...
  constructor() {
    this.baseText = `${sharpyy("XERNERX", "txRainbow", "bold")} | ${new Date()
      ...
      .join(
        " ",
      )} | ${sharpyy(String(Math.round(process.memoryUsage().rss / 10000) / 100), "txCyan")}mb |`;
  }
  ...
  async error(message: string, error?: Error) {
    ...
    if (error)
      return console.error(`❗ |`, this.baseText, message, errorMessage);
  ...
  warn(message: string, url?: URL) {
    ...
    return console.warn(
      ...
      this.baseText,
      message,
      url ? `Read more here ${link("here", url)}` : "",
    );
  }
