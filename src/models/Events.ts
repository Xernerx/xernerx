                ...
                commands
                    ...
                    .map(async (cmd) => {
                        ...
                        if (
                            ...
                            (cmd.prefix.includes(px as string) || prefix?.includes(px) || px === 'MentionPrefix')
                        ) {
                            try {
                                ...
                                if ((cmd.conditions as unknown) && ((await cmd.conditions(message, (await messageArguments(message, cmd)) as unknown as MessageArgumentOptions)) as unknown)) return;

                                await cmd.exec(message, (await messageArguments(message, cmd)) as unknown as MessageArgumentOptions);
            ...
            if (interaction.type === InteractionType.ApplicationCommand) {
                if (this.client.commands.context.has(interaction.commandName)) {
                    ...
                    try {
                        ...
                        if (command.type === 3) args = await interaction.channel.messages.fetch(interaction.targetId);

                        if (command.type === 2) args = await this.client.users.fetch(interaction.targetId);
