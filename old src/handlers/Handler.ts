...
export default class Handler {
    ...
    constructor(client: XernerxClient) {
        ...
        this.commands = Array<MessageCommandBuilder | SlashCommandBuilder | ContextCommandBuilder>;
    ...
    public async load(dir: string, fileName: string, type: string) {
        try {
            dir.split(/\//).shift();

            let file = (await import('file://' + `${path.resolve(dir)}/${fileName}`)).default;
            ...
            file.filepath = `${path.resolve(dir)}\\${file}`;

            file.file = file;

            if (type === CommandType.MessageCommand) this.client.commands.message.set(file?.name || file?.name, file);

            if (type === CommandType.SlashCommand) {
                this.client.commands.slash.set(file?.data?.name || file?.name, file);
            }

            if (type === CommandType.ContextCommand) {
                this.client.commands.context.set(file?.data?.name || file?.name, file);
            }
            ...
            if (type === CommandType.Inhibitor) {
                this.client.inhibitors.set(file.name, file);
            }
        } catch (error) {
            console.error(Style.log(`Xernerx | Couldn't load ${fileName} because <${error}>`, { color: Style.BackgroundColor.Red }));
        }
    }
}
