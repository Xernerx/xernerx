...
export default class XernerxClient extends Client {
    ...
    config;
    constructor(discordOptions, clientOptions, config) {
        ...
        if (this.settings?.log?.ready) {
            this.once('ready', async (client) => {
                ...
                console.info(Style.log(`Xernerx | ${client.user.tag} signed in watching ${size} server${size > 1 ? 's' : ''}. ${(this.handlerOptions.slash || this.handlerOptions.context)?.guildId
