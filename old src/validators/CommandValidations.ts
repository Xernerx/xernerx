...
export default function commandValidation(
    ...
    if (client.settings.ignore?.owner && client.util.isOwner(user.id)) return res;
        ...
        const permissions = command.permissions?.user || (client?.settings?.permissions?.user as unknown as bigint[])?.push(...handlerPermissions(client, command.commandType, 'user')),
            missing: bigint[] = [];
        ...
        const permissions = command.permissions?.client || (client?.settings?.permissions?.client as unknown as bigint[])?.push(...handlerPermissions(client, command.commandType, 'client')),
            missing: bigint[] = [];
